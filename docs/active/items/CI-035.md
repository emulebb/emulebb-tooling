...
stays Open because the quick final proof gates and final package hash record
are incomplete.
...
- [ ] Main app package manifests and per-file hashes confirm that legacy
      template-based `eMule/webserver` payload is absent from RC assets.
...
The release campaign composes the quick blocking UI, REST, live-network, Arr,
aMuTorrent, stabilization, and crash-smoke lanes while keeping full overnight
and real-profile monitoring as supporting soak evidence.
