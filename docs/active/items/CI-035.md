...
stays Open because the full final proof and final package hash record are
incomplete.
...
documented RC identity before final package proof:
...
Final fast certification remains incomplete until the `hide.me` interface is
available again and the certification command is rerun. This is not accepted
final proof yet.
