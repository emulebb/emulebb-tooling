...
Preserved does not mean supported. For `0.7.3` and later `0.7.x` maintenance,
some frozen legacy code, resource entries, profile keys, or old menu paths may
remain so the release can preserve compatibility while keeping risk low. That
retention is a transition state, not a promise that the surface will receive
fixes, documentation, automation, or release proof.

`0.8.0` is the explicit frozen-surface removal release. Frozen areas listed
below should be removed or replaced through tracked `0.8.0` roadmap/refactor
items, with proof focused on supported behavior that shares infrastructure.
