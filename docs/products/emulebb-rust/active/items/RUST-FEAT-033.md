...
| Early network connection | PARTIAL | Current persisted soak samples show ED2K connected with High ID and Kad connected. On 2026-07-22 after the orchestrated regular rebuild, `rust-p2p-start` forced startup immediately: ED2K connected to the operator-selected server with HighID, and Kad reached connected state on the next short sample. Convert this into a repeatable early-connect gate that fails fast instead of waiting for long soak windows. |
