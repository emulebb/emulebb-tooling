...
1. Keep the sanitized `public-search-download-proof`, `rust-webui-live-proof`,
   and `shared-reload-settled-proof` harnesses on the regular daemon path as the
   current download/search, embedded WebUI, and shared-library gates.
