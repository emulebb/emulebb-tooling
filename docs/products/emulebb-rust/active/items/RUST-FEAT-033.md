...
| Regular logs and diagnostics | PARTIAL | The repeatable `repos/emulebb-build-tests/scripts/rust-soak-control.py regular-log-proof` gate now writes a sanitized retained report and exits nonzero when beta diagnostic categories are missing. On 2026-07-22 it passed against the regular persisted daemon and wrote `reports/rust-regular-log-proof/rust-regular-log-proof.latest.json`: the 500-entry regular log ring contained VPN Guard HTTP public IPv4 probes, STUN probes, ED2K, Kad, publish, and download categories, while `/status` plus persisted metadata proved ED2K HighID, Kad connected, 589932079 uploaded bytes, 2950 upload accepts, 46951 completed known files, and 46963 transfers. The same proof exposed the remaining product-side weakness: recent regular logs had no startup or upload log entries because publish/traversal activity can rotate them out. Before tag, harden regular log retention or periodic summary logging so a recent regular-exe excerpt itself diagnoses startup and upload state without falling back to diagnostics binaries. |
