...
into a clean restart/reload proof after hashing settles, while extending WebUI
live proof over the completed public search/download workflow.
Work this as small slices:

1. Keep the sanitized `public-search-download-proof` harness on the regular
   daemon path as the current download/search gate and reuse the strict report
   for WebUI workflow coverage.
