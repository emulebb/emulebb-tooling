...
| Public upload path | PARTIAL | The regular persisted daemon showed public active uploads with non-zero speed before the 2026-07-22 shared-root recovery. After stale slots were cleared, the public profile is connected but visibility-limited while ED2K/Kad republish restored files. Deterministic regular-exe Rust-Rust upload proof passed at `rust-local-upload/20260722T162200Z` with 4 MiB delivered and max upload speed 272.672 KiB/s; keep monitoring for fresh public upload throughput. |
