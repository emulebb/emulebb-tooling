...
from byte-progress proof into finished-file proof, because local
download/resume/delivery has fresh deterministic evidence while
...
1. Keep the sanitized `public-search-download-proof` harness on the regular
   daemon path and feed it public candidates until it records completed
   delivery, while continuing strict complete-source probes for a cleaner gate.
