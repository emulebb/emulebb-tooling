...
|----------|-------|----------|
| `Alt+1` | main shell | Opens Kad. |
| `Alt+2` | main shell | Opens Servers. |
| `Alt+3` | main shell | Opens Transfers. |
| `Alt+4` | main shell | Opens Search. |
| `Alt+5` | main shell | Opens Shared Files. |
| `Alt+6` | main shell | Opens Messages. |
| `Alt+7` | main shell | Opens IRC. |
| `Alt+8` | main shell | Opens Statistics. |
| `Alt+9` | main shell | Opens Options. |
...
| `Alt+O` | main shell | Opens Options as a command alias for `Alt+9`. |
...
- The old hidden hotmenu `Alt+X` button was retired; floating hotmenu ownership
  is now `Alt+U`.
- Primary toolbar navigation uses `Alt+1` through `Alt+9` instead of toolbar
  label letters. This avoids letter collisions between localized labels and
  baseline English labels such as `Transfers`.
- Tools uses `Alt+W`; `Alt+T` is intentionally not claimed by the app shell so
  it cannot conflict with Transfers.
...
- With Prompt on exit enabled, `Alt+X` should show the existing confirmation
  before shutdown.
- `Alt+1` through `Alt+9` should open Kad, Servers, Transfers, Search, Shared
  Files, Messages, IRC, Statistics, and Options respectively.
- `Alt+U` should open the floating hotmenu.
- `Alt+W` should open the Tools popup.
- `Alt+O` should open Options as an alias.
- `Alt+T` should not open Tools and should not steal Transfers focus.
